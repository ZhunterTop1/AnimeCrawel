...
public abstract class Aapplication {
     ...
     * */
    public void start(String url,String aname,Map<String, String> cookie,boolean useLog,Class cz,boolean isList,List<Integer> list){
        IEnvBuilder builder = new EnvBuilder().setUrl(url).setAname(aname).setUseLog(useLog).setCookie(cookie).setDriverClass(cz).isList(isList).list(list).builder();
